...
const ChatWindowAdminOfficial = ({ userId, userRole }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (userRole === 'admin') {
        const { data } = await supabase.from('profiles').select('id, name').eq('role', 'official');
        setUsers(data || []);
      } else if (userRole === 'official') {
        const { data } = await supabase.from('profiles').select('id, name').eq('role', 'admin').single();
        setUsers(data ? [data] : []);
      ...
      <div className="w-full max-w-4xl bg-white/10 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden">
        <div className="grid grid-cols-3">
          ...
          <div className="col-span-1 bg-[#1D3557] p-6 text-white border-r border-white/10">
            <h2 className="text-2xl font-bold mb-6">{userRole === 'admin' ? 'Officials' : 'Admin'}</h2>
              ...
              {users.map((u) => (
                <li
                  ...
                  className={`p-4 rounded-lg cursor-pointer transition ${
                    selectedUser?.id === u.id ? 'bg-[#2E5A88]' : 'bg-white/10 hover:bg-[#2E5A88]'
                  }`}
                >
                  {u.name}
                </li>
          ...
          <div className="col-span-2 flex flex-col h-[80vh]">
            <div className="flex-1 p-6 overflow-y-auto space-y-4">
              {messages.map((msg) => (
                ...
                >
                  <p className="text-sm font-semibold mb-1">{msg.sender?.name || 'User'}</p>
                  <p className="mb-1">{msg.content}</p>
                  <p className="text-xs text-gray-300 text-right">{format(new Date(msg.created_at), 'hh:mm a')}</p>
                </div>
              ))}
            ...
            {/* Message Input */}
            <form onSubmit={sendMessage} className="flex items-center p-4 border-t border-white/10 bg-white/10">
              <input
                type="text"
                value={message}
