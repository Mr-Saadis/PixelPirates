...
import { FaUser, FaSignOutAlt, FaMapMarkerAlt, FaClipboardList, FaComments, FaChartPie, FaPlus } from 'react-icons/fa';
        ...
        <div className="flex min-h-screen bg-[#f0f4f8]">
            ...
            <aside className="w-64 bg-[#1D3557] text-white flex flex-col justify-between">
                <div>
                    ...
                    <nav className="flex flex-col gap-2 p-4">
                        <SidebarItem icon={<FaPlus />} label="Report Issue" onClick={() => router.push('/report')} />
                        <SidebarItem icon={<FaClipboardList />} label="My Issues (List)" onClick={() => router.push('/my-issues/list')} />
                ...
                <div className="p-4 border-t border-white/20">
                    <SidebarItem icon={<FaUser />} label="Profile" onClick={() => router.push('/dashboard/profile')} />
            ...
            <main className="flex-1 p-6">
                <header className="flex justify-between items-center mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-[#1D3557]">Welcome back, {userName || 'Citizen'}</h1>
                        <p className="text-sm text-gray-600">Here’s a snapshot of your activity</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="text-right">
                            <p className="text-sm font-semibold text-[#1D3557]">{userName || 'Citizen'}</p>
...
function SidebarItem({ icon, label, onClick }) {
    return (
        <button onClick={onClick} className="flex items-center gap-3 px-3 py-2 rounded hover:bg-white/10 transition-all w-full text-left">
            {icon}
            <span>{label}</span>
        </button>
    );
